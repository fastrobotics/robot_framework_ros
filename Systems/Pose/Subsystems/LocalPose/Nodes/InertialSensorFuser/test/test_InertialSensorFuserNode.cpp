...
void machine_inertial_Callback([[maybe_unused]] const sensor_msgs::Imu& msg) { machine_inertial_rx_count++; }
TEST(InertialSensorFuserNode, TestBasics) {
    ros::NodeHandle nh("~");
    ...
    std::string machine_inertial_topic = robot_namespace + "/imu";
    ros::Subscriber machine_inertial_sub = nh.subscribe(machine_inertial_topic, 100, &machine_inertial_Callback);

    ros::Publisher imu_pub = nh.advertise<sensor_msgs::Imu>(robot_namespace + "/imu1", 1);
    sleep(5.0);
    ...
    ASSERT_EQ(1, machine_inertial_sub.getNumPublishers());
    ASSERT_EQ(1, imu_pub.getNumSubscribers());

    sleep(1.0);  // Wait for InertialSensorFuserNode to Start.
    ...
    ASSERT_TRUE(machine_inertial_rx_count > 0);
}
int main(int argc, char** argv) {
    testing::InitGoogleTest(&argc, argv);
    ros::init(argc, argv, "test_InertialSensorFuserNode");
