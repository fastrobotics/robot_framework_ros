...
std::string unittest_nodename = "nodeTrajectoryController";
uint64_t heartbeat_rx_count = 0;
uint64_t diagnostic_rx_count = 0;
uint64_t ready_to_arm_rx_count = 0;
uint64_t command_rx_count = 0;
robot_framework_ros::ready_to_arm ready_to_arm_data;
...
void ready_to_arm_Callback(const robot_framework_ros::ready_to_arm& msg) {
    ready_to_arm_data = msg;
    ready_to_arm_rx_count++;
}
void command_Callback([[maybe_unused]] const geometry_msgs::Twist& msg) { command_rx_count++; }
TEST(TrajectoryControllerNode, TestBasics) {
    ros::NodeHandle nh("~");
    ...
    std::string pose_topic = robot_namespace + "/pose";
    ros::Publisher pose_pub = nh.advertise<nav_msgs::Odometry>(pose_topic, 1);

    std::string desired_command_topic = robot_namespace + "/desired_command";
    ros::Publisher desired_command_pub = nh.advertise<geometry_msgs::Twist>(desired_command_topic, 1);

    std::string command_topic = robot_namespace + "/command";
    ros::Subscriber command_sub = nh.subscribe(command_topic, 100, &command_Callback);
    ...
    ASSERT_EQ(1, pose_pub.getNumSubscribers());
    ASSERT_EQ(1, desired_command_pub.getNumSubscribers());
    ASSERT_EQ(1, command_sub.getNumPublishers());

    sleep(1.0);  // Wait for TrajectoryControllerNode to Start.
    ...
    while (current_time < 3.0) {
        ...
        cmd_data.angular.z = 1.0;
        desired_command_pub.publish(cmd_data);
    ...
    ASSERT_TRUE(command_rx_count > 0);
    ASSERT_TRUE(ready_to_arm_data.ready_to_arm);
}
int main(int argc, char** argv) {
    testing::InitGoogleTest(&argc, argv);
    ros::init(argc, argv, "test_TrajectoryControllerNode");
