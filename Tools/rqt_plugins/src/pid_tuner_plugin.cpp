...
namespace robot_framework_ros {
    ...
    void PIDTunerPlugin::knobSetpointChanged(int value) { latest_setpoint_ = (double)value; }
    ...
    void PIDTunerPlugin::initPlugin(qt_gui_cpp::PluginContext& context) {
        ...
        if (container) {
            ...
            axis_y_->setRange(-1.0, 1.0);  // Set expected robot limits
        ...
        dial_setpoint_ = loaded_layout->findChild<QDial*>("dial_Setpoint");
        if (dial_setpoint_) {
            connect(dial_setpoint_, &QDial::valueChanged, this, &PIDTunerPlugin::knobSetpointChanged);
        }
